...
async def process_file_background(file_path: Path, original_filename: str, task_id: str):
    ...
    try:
        ...
        task_status[task_id]["output_file"] = str(processed_filepath)  # store file path
...
async def get_result(task_id: str):
    """Return the processed JSON file for the given task ID."""
    if task_id not in task_status:
        raise HTTPException(status_code=404, detail="Task not found")
    ...
    if task_info["status"] != "completed":
        raise HTTPException(status_code=400, detail=f"Task is {task_info['status']}, result not available yet")

    output_file = task_info.get("output_file")
    if not output_file or not Path(output_file).exists():
        raise HTTPException(status_code=404, detail="Processed file not found")

    # Option 1: Return as downloadable JSON file
    return FileResponse(output_file, media_type="application/json", filename=Path(output_file).name)

    # Option 2: Return parsed JSON directly (uncomment below if preferred)
